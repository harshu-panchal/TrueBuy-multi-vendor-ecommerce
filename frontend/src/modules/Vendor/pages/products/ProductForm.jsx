...
const ProductForm = () => {
  ...
  const [initialWholesale, setInitialWholesale] = useState({
    isWholesale: false,
    minOrderQty: 1,
    bulkPricingJson: "[]",
  });
  ...
  const populateForm = (product, cats) => {
    ...
    const rawBulk = Array.isArray(product.bulkPricing) ? product.bulkPricing : [];
    const normalizedBulkNumeric = rawBulk
      .map((t) => ({
    ...
    setInitialWholesale({
      ...
      bulkPricingJson: JSON.stringify(normalizedBulkNumeric),
    });
    ...
    setFormData({
      ...
      bulkPricing: Array.isArray(product.bulkPricing)
        ...
        : [],
      wholesaleApprovalStatus: product.wholesaleApprovalStatus || "",
      wholesaleRejectionReason: product.wholesaleRejectionReason || "",
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const wholesaleChanged =
      Boolean(formData.isWholesale) !== Boolean(initialWholesale.isWholesale) ||
      (formData.isWholesale
        ? (Number(parsedWholesaleMoq) !== Number(initialWholesale.minOrderQty) ||
          JSON.stringify(normalizedBulkPricing) !== String(initialWholesale.bulkPricingJson || "[]"))
        : false);

    // Only send wholesale fields if they actually changed (prevents unnecessary re-approval on normal edits).
    if (!isEdit) {
      if (formData.isWholesale) {
    ...
    } else if (wholesaleChanged) {
      payload.isWholesale = Boolean(formData.isWholesale);
      if (formData.isWholesale) {
        payload.minOrderQty = parsedWholesaleMoq;
        payload.bulkPricing = normalizedBulkPricing;
        payload.visibleTo = "vendors";
        ...
        <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-200">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {formData.isWholesale && formData.wholesaleApprovalStatus && (
                <div className="mt-1 text-[11px] text-gray-600">
                  Status: <span className="font-semibold">{String(formData.wholesaleApprovalStatus)}</span>
                  {String(formData.wholesaleApprovalStatus).toLowerCase() === "rejected" && formData.wholesaleRejectionReason
                    ? ` • Reason: ${formData.wholesaleRejectionReason}`
                    : ""}
                </div>
              )}
            </div>
